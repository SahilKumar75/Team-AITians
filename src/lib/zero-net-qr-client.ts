...
export interface PatientEmergencyInput {
...
function sanitizeVCardValue(value?: string | null): string {
  return (value || "")
    .replace(/\\/g, "\\\\")
    .replace(/\n/g, "\\n")
    .replace(/,/g, "\\,")
...
function compactList(value?: string | null, fallback = "None"): string {
  const items = (value || "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
...
/**
 * Build a standards-based VCARD payload for broad camera compatibility (iOS + Android).
 * This allows immediate offline scan display in native camera contact cards.
 */
export function buildEmergencyVCard(
  patientData: PatientEmergencyInput,
  emergencyUrl?: string
): string {
  const fullName = (patientData.fullName || "Unknown Patient").trim();
  const bloodGroup = (patientData.bloodGroup || "Unknown").trim();
  const allergies = compactList(patientData.allergies, "None reported");
  const medications = compactList(patientData.currentMedications, "None reported");
  const conditions = compactList(patientData.chronicConditions, "None reported");
  const emergencyName = (patientData.emergencyName || "N/A").trim();
  const emergencyPhone = (patientData.emergencyPhone || "").trim();
  const walletAddress = (patientData.walletAddress || "").trim();
  const displayName = `${fullName} | BG ${bloodGroup}`;
  ...
  const noteLines = [
    ...
    "Generated by Swasthya Sanchar",
  ].filter(Boolean);

  const lines = [
    "BEGIN:VCARD",
    "VERSION:3.0",
    `FN:${sanitizeVCardValue(displayName)}`,
    `N:${sanitizeVCardValue(fullName)};;;;`,
    `TITLE:${sanitizeVCardValue(`Blood Group: ${bloodGroup}`)}`,
    `ORG:${sanitizeVCardValue(`Emergency Contact: ${emergencyName}`)}`,
    emergencyPhone ? `TEL;TYPE=ICE,CELL:${sanitizeVCardValue(emergencyPhone)}` : "",
    emergencyUrl ? `URL:${sanitizeVCardValue(emergencyUrl)}` : "",
    `NOTE:${sanitizeVCardValue(noteLines.join("\n"))}`,
    "END:VCARD",
  ].filter(Boolean);
...
/**
 * Encode emergency profile in browser-safe Zero-Net format (no Node crypto dependency).
 */
export function encodeEmergencyProfileClient(patientData: PatientEmergencyInput): string {
  ...
  const issuedAt =
    typeof patientData.issuedAtUnix === "number" && Number.isFinite(patientData.issuedAtUnix)
      ? Math.floor(patientData.issuedAtUnix)
      : Math.floor(Date.now() / 1000);

  const profile: EmergencyProfile = {
    v: 1,
    n: (patientData.fullName || "").trim(),
    d: dob,
    g: ((patientData.gender || "U").toString().charAt(0) || "U").toUpperCase(),
    b: (patientData.bloodGroup || "Unknown").trim(),
    a: split(patientData.allergies),
    m: split(patientData.currentMedications),
    c: split(patientData.chronicConditions),
    e: {
      n: (patientData.emergencyName || "").trim(),
      r: (patientData.emergencyRelation || "").trim(),
      p: (patientData.emergencyPhone || "").trim(),
    },
    w: (patientData.walletAddress || "").trim(),
    t: issuedAt,
  };

  const msgpacked = msgpackEncode(profile);
  const compressed = pako.gzip(msgpacked);
  const b64 = bufferToBase64Url(compressed);
